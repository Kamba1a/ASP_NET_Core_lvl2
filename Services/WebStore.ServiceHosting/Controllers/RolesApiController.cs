...
namespace WebStore.ServiceHosting.Controllers
{
    ...
    {
        private readonly RoleStore<Role> _RoleStore; // по аналогии с User-контроллером, здесь через контроллер реализуются все методы RoleStore
        ...
        [HttpPost]
        public async Task<bool> CreateAsync(Role role) => (await _RoleStore.CreateAsync(role)).Succeeded; //!! клиенту не возвращаем полный IdentityResult, а только успех/не успех
