...
namespace WebStore.Clients.Identity
{
    public class UsersClient : BaseClient, IUserClient
    {
        ...
        //в методах "Set...", где передаются данные пользователя, прежде чем передать контроллеру новое устанавливаемое значение, надо сначала внести изменения на стороне клиента (обязательно)
        ...
        {
            user.UserName = name;   //прежде чем передать контроллеру новое значение, надо сначала внести изменения на стороне клиента (обязательно)
