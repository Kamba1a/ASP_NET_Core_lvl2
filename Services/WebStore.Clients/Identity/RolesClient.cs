    ...
    public class RolesClient : BaseClient, IRolesClient
    {
        ...
        public async Task<IdentityResult> CreateAsync(Role role, CancellationToken cancel) =>
            await (await PostAsync(_ServiceAddress, role, cancel))
               .Content
               .ReadAsAsync<bool>(cancel) //т.к. контроллер не перадет полный IdentityResult, прописываем сами в зависимости от пришедшего bool значения
                ? IdentityResult.Success : IdentityResult.Failed(); //при этом возможный список ошибок в IdentityResult останется на стороне контроллера, без передачи клиенту (безопасность)
